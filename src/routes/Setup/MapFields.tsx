...
export const MapFields: React.FC<MapFieldsProps> = ({
  ...
  useEffect(() => {
    if (modelFields) {
      ...
      modelFields.forEach((field) => {
        const similarities: { param: string; score: number }[] = params.map(
          (p) => ({
            param: p.name,
  ...
  const handleSelectParam = (
    einsteinFieldName: string,
    tableauParamName: string
  ) => {
    const mapIndex = mappedFields
      .map((f) => f.einFieldName)
      .indexOf(einsteinFieldName);
  ...
  const getDropdownValue = (einsteinFieldName: string): string => {
    const matchedFields = mappedFields.filter(
      (f) => f.einFieldName === einsteinFieldName
    );
      ...
      <div css={cssFieldMapContainer}>
        <div css={cssFieldMapRowContainer(0)}>
          <div
            ...
            `}
          >
            {components.tables.fieldMap.headers.einsteinModelParams}
          </div>
          <div
            css={css`
              flex: 1;
            ...
            `}
          >
            {components.tables.fieldMap.headers.tableauParams}
          </div>
        </div>
            ...
            >
              <DropdownSelect
                ...
                onChange={({ target: { value } }) =>
                  handleSelectParam(fieldName, value)
                }
                css={css`
                  min-width: 300px;
                `}
