...
export const PredictionDefs: React.FC<PredictionDefsProps> = ({
  auth,
  onPredictionDefSelected,
  onBack,
  language = 'en',
}) => {
  ...
  useEffect(() => {
    if (predictionDefs) {
      if (predictionDefs.length > 0) {
        ...
        alert(
          "No prediction definitions were found in your Einstein account! You must deploy your model and the model must show up in Model Manager before you'll be able to use this extension."
        );
      }
    }
  }, [predictionDefs]);
  ...
  const { messages, components } = getStringsByLanguageCode(
    language
  ).strings.setup.selectPredictionDefinition;
        ...
        content: (
          <React.Fragment>
            <Button
              kind="primary"
              onClick={() =>
                selectedDef ? onPredictionDefSelected(selectedDef) : null
              }
              css={css`
                margin-right: 20px;
              `}
    ...
    >
      ...
      >
        <DropdownSelect
          ...
          label={components.selects.predictionDefs.label}
          kind="line"
        >
          {predictionDefs.map((pd) => (
            <option key={pd.url} value={pd.id}>
              {pd.label}
            </option>
          ))}
        </DropdownSelect>
