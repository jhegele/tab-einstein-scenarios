...
export const Explain: React.FC<ExplainProps> = ({
  ...
  const middleValueColToExplanation = (
    col: SFDCColumn,
    ...
    if (matchRange) {
      const rangeMin: string = numeral(parseFloat(matchRange[1])).format(
        prefs.explain.explanationNumberFormatting
      );
  ...
  const downArrow = (
    <ArrowIcon variant="down" color={prefs.explain.arrowDownColor} />
  );
    ...
    >
      <thead>
        <tr
          css={css`
            & > th {
              border-bottom: 2px solid ${prefs.global.textColor};
            }
          `}
        >
          <th>&nbsp;</th>
          <th
            ...
            `}
          >
            {components.tables.explainResults.headers.value}
          </th>
          <th>{components.tables.explainResults.headers.explanation}</th>
        </tr>
      </thead>
      <tbody>
        {middleValues.map((mv, idx) => (
          <tr
            css={
              idx < middleValues.length - 1
                ? css`
                    & > td {
                      border-bottom: 1px dotted ${prefs.global.textColor};
                    }
                  `
                : null
            }
          ...
          >
            ...
            >
              {mv.value > 0 ? '+' : ''}
              {numeral(mv.value).format(prefs.explain.valueNumberFormatting)}
            </td>
            <td>
              <table>
                <tbody>
                  {mv.columns.map((col, idx) => (
                    <tr key={`pred.mv.col.${idx}`}>
                      <td>
                        {middleValueColToExplanation(
                          col,
                          idx < mv.columns.length - 1
                        )}
                      </td>
                    </tr>
                  ))}
