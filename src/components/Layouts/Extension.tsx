...
const cssDisplayMenuItem = (
  pageBorderBottomColor: string,
  isActive: boolean
) => css`
  flex: 1;
  display: flex;
...
export const LayoutExtension: React.FC<LayoutExtensionProps> = ({
  ...
  const handleClickToolbar = () => {
    ...
    ui.displayDialogAsync(url, JSON.stringify(payload), {
      width: 800,
      height: 600,
    })
      ...
      .catch((err) => {
        console.log(
          "A dialog display error occurred. It's likely that this is simply the user closing the dialog manually. The reported error is logged below."
        );
        console.log(err);
      });
  };
  ...
  if (showToolbar) {
    toolbar = (
      <div css={cssToolbar} onClick={handleClickToolbar}>
        <FiSettings
          css={css`
            color: #fff;
      ...
      <div css={cssDisplayMenu}>
        {pages.map((page) => (
          <div
            ...
            css={cssDisplayMenuItem(
              preferences.global.accentColor,
              page.active
            )}
    ...
    <React.Fragment>
      <div
        css={cssOuterContainer(
          preferences.global.backgroundColor,
          preferences.global.textColor
        )}
      >
        {getPageMenu()}
